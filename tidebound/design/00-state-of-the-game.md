...
| 49 endings | **33 CORES endings** (RESCUE, STAY_OPEN, BROKER, HOME, VILLAGE, JOIN, KEEPER, COVENANT, TWO_WORLDS, SAIL_BLESSED, RYO_BOAT, LONG_SWIM, WHOLE_SKY, COCO, THREE_SPRINGS, LAST_PACK, TRICKSTER, ROSAS_RANSOM, OTHER_SIGNAL, FIRST_KAARI, + the Endings Expansion in ch7: REGRET, EMPTY_HORIZON, CARTOGRAPHER, REMAIN, HERMIT_HEIR, ILSA_ANSWER, DROWNED_DOOR, TIDE_PRICE, WIND_TAKES, SOUNDER, ROOSTER_DAWN, NINES_GARDEN, ALONE_UNBROKEN) + 8 death endings (thirst, hunger, injury, undertow, fever, grin, despair, dark) | Ending engine is parameterized — new endings = CORES entry (scenes-extra.js appends via TB.CORES) + convergence mapping. Now 39 cores: Phase-3 jokes (CRAB_TOWN, COCONUT_MOGUL), X3 The Loop (NG+), S7 WHAT_REMAINS (via the Scars-Not-Graves peril arc — PERIL_HEALED/PERIL_SELFHEALED, not a death), ISLANDS_OWN "The Island's Own" (the owner's own idea: kindred companion trust≥90 + TIDEWELL_WITNESS/FEED → walk the companion up the mountain and hand the covenant to the one it always fit; its `body` is a FUNCTION (per-companion keeper prose — the ending scene resolves `typeof core.body === 'function'`); companionCovered; special Edda epilogue beat "It was waiting for you to introduce them"), and LAST_DELIVERY "The Last Delivery" (the courier payoff's capstone: CASE_OPEN + COURIER_RESTED + a way out → leave as a courier, drown Meridian's dossier, deliver the photograph to the pier at KAI—; body-as-function varies the gems' fate). COURIER'S PAYOFF layer (scenes-extra + map + ch6/7): '🕯️ Build the courier a cairn' hub action once CASE_OPEN && almanac frags 9/9 → `courier_rest` (COURIER_RESTED, hope+8, per-companion + Edda's candle beats, epilogue line, ledger road if skipped); grotto expedition takes region-priority `runGemsReturn` when GEMS && ch≥4 (GEMS_RETURNED, hope+6, +1 regard — same weight as OLD_THINGS_HOME); trophy The Last Mile (shelf 29). THE WAYS IT ENDS (scenes-ways.js + edits): now 41 cores & 12 deaths. New deaths, ALL warned-choice, never RNG: 'boarking' (⚔️ hunt option on ev2_boarking — survives mauled to `ev2_boarkface`/KING_FACED only if health≥60 && energy≥45 && kavi/buri), 'fall' (`cliff_ledge` egg-ledge scene, routed by 2nd cliffs expedition — dies iff energy<30 || ch5 monsoon, else CLIFF_LEDGE + food), 'coldfire' (ev5_cyclone forks ONLY for shelter≤1 && !overhang: burrow survives, ⚠️ stay dies), 'ash' (ch6_tremor forks: bedrock camp → ch6_inner, ⚠️ night push dies). New cores: LIGHTKEEPER (S4 — 3rd cliffs expedition finds the Kaari fire-tower `runTowerFind`/TOWER_FOUND, 3-stage '🗼' hub action → TOWER_BUILT, convergence door when !TIDEWELL_SILENCE) and HUM_SILENCED (M3 — gate !TIDEWELL_SILENCE && (WOUND_SEEN||HEARTGLASS||GULLET2||SUNDERING_SEEN) && depth≥15; flat, non-editorializing card; counts as leaving). Death cards in ch1 death scene; titles in runcard DEATH_TITLES. Still unbuilt from design/09: D9/D10 death causes. C2 Good Boy is VETOED by the owner — companions never die; peril arcs are the sanctioned substitute. |
