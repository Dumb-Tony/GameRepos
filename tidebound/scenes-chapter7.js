...
(function (G) {
  ...
  TB.scene('convergence', {
    ...
    choices: (s) => {
      ...
      if (!TB.is('TIDEWELL_SILENCE')) {
        ...
        if (s.companion && (TB.is('PERIL_HEALED') || TB.is('PERIL_SELFHEALED'))) c.push({
          t: '🩹 Stay — you nearly lost this once. You know what it\'s worth now.', sub: 'What Remains. Home is a heartbeat you can check on.',
          do: () => { TB.state.endingId = 'WHAT_REMAINS'; }, go: 'ending' });
  ...
  const CORES = {
    ...
    WHAT_REMAINS: { icon: '🍂', title: 'WHAT REMAINS', bg: 'camp-fringe', body: [
  ...
  function epilogue(s, id) {
    ...
    if (TB.is('M_VIGIL_DONE') && id !== 'OTHER_SIGNAL') t.push(leaving
      ? '— And you take the pact out with you: every seventh night, one hour, wherever in the world you live, a receiver hisses on a frequency no chart admits — and some nights the skips align, and nine faint beats cross the whole hidden sea to find you. You never miss the hour. Neither, in eleven-plus years, has she.'
      : '— And the pact holds: every seventh night, one hour, your midnight — two lighthouses on one hidden sea, trading telegrams, storm warnings, and once a year, on no schedule either of you will admit to, the sound of the other one laughing.');
    if (TB.is('TWELVE_HEARD') && id !== 'OTHER_SIGNAL') t.push(leaving
      ? '— And on the first vigil of every month, wherever you are, you keep the twelfth hour too: the dead station that answered, the new keeper learning their hands. Three lighthouses now, and the roll still being called. You left the island. Nobody leaves the Archipelago.'
      : '— And the twelfth hour stays kept: first vigil of every month, three stations now where for nine years there was grief with a schedule. The new keeper\'s beats get steadier every month. One day they\'ll be teaching someone. That\'s the whole job, passed hand to hand across a sea no chart will ever hold.');
    else if (TB.is('OTHER_HEARD') && id !== 'OTHER_SIGNAL' && !leaving) t.push('— And some nights, at the radio, in the skips: the nine-beat station, keeping her vigil across the hidden world. You are two lighthouses who know each other\'s light.');
