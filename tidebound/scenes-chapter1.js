...
(function (G) {
  ...
  TB.scene('death', {
    ...
    text: (s) => {
      ...
      else if (cause === 'grin') { t.push('<em>ENDING — OLD GRIN</em>', 'The mangroves have one landlord, and you knew his terms, and you were sure — swift-water sure, spear-in-hand sure, too-far-to-turn-back sure — that terms were negotiable.', 'It is very fast. The water closes like a ledger. Far upriver the herons resettle, the crabs resume their offices, and the oldest patience on the island slides back under the tea-dark water, undefeated, unhurried, owed nothing now at all.', TB.is('EDDA_MET') ? 'Edda\'s eulogy, delivered to her garden, is four words: "Told it. Didn\'t listen."' : 'Somewhere up the mountain, a woman you never got to meet hears the herons resettle, reads the whole story in it, and says four words to her garden: "Told it. Didn\'t listen."'); }
      else if (cause === 'boarking') { t.push('<em>ENDING — THE BOAR KING</em>', 'You went to him with a week\'s knowledge of this island, a sharpened stick, and the oldest human error there is: mistaking your own decision for readiness.', 'He was not cruel about it. Veterans rarely are. There was one warning — the true kind, the lowered head, the single grounded stamp you didn\'t know how to read — and then the inland dark\'s whole arithmetic arrived at once, and the ledger of your hundred days closed at seven.', 'His epilogue card is the only one of its kind, because the King keeps his own Ledger, and it is very old, and you are in it now: <em>the fourth</em>. The first three also thought the rent was negotiable. The wall of the wallow holds four notches, and he makes them himself, with the broken tusk, slowly, because a treaty-keeper records what he is owed and what it cost.'); }
      else if (cause === 'fall') { t.push('<em>ENDING — THE FALL</em>', 'The eggs were real. The ledge was real. Your arms, at the end of the day you\'d already spent, were a rumor you chose to believe.', 'The cliffs at Kestrel are honest the way the sea is honest: they publish their terms in guano-slick stone and wind-shear and forty visible meters of consequence, and they never once pretended the rail was optional. You pretended it for them.', (s.companion === 'vela' ? 'Vela\'s people' : 'The sea eagles') + ' ride the updraft over the point for three days after, lower than they ever ride. The islanders of the air, paying what respect the air can pay. The cache of eggs stays where it was, warm-side to the sun, and every one of them hatches.'); }
      else if (cause === 'coldfire') { t.push('<em>ENDING — COLD FIRE</em>', 'No single moment killed you. That is the whole of this card, and the island would want it stated plainly: not the cyclone, not the cold, not the dark. What killed you was a season of small skipped choices arriving all at once to be paid — the shelter tier you meant to get to, the firewood store that stayed a plan, the overhang you didn\'t move to on Day 4 because the beach had a view.', 'The storm only did the audit.', 'They find the camp in the spring — ' + (TB.is('RYO_MET') ? 'Ryo, if he lives; strangers, if not' : 'strangers, whoever the sea sends') + ' — and the strange thing, the thing that stops them, is how CLOSE it all was: the half-built windbreak, the stacked stones, the good intentions legible in every unfinished thing. The island files it under its second-oldest heading: <em>tomorrow.</em>'); }
      else if (cause === 'ash') { t.push('<em>ENDING — MOTHER ASH</em>', 'The mountain told you. That is the unbearable mercy of it — the island that never warns twice warned you all day: the ladder of tremors, each rung closer, the birds gone from the whole face of the crown, ' + (TB.is('NAIA_MET') ? 'Naia\'s people' : 'the mountain\'s own people') + ' waiting for daylight with four hundred years of reasons. You climbed into the dark because the summit was CLOSE, and close, on a wounded mountain, is a direction, not a distance.', 'The crown shrugged once more — the big one, the one the whole ladder was counting toward — and the crack-new rock you were crossing stopped being rock.', 'It is the island\'s one unsurvivable tantrum, and even so, it wasn\'t rage. Wounded things startle. The Kaari have a word for dying this way, and it is not an insult, but it is not a kind word either: it translates, roughly, as <em>early</em>.'); }
      ...
      // an early grave earns a quiet signpost to the gentler setting
      if (s.day <= 20 && s.mod !== 'kind' && !(TB.Audio && TB.Audio.settings().kind)) t.push('<span class="game-sub">(If the island\'s arithmetic is winning: Menu → Kind tide softens the meters for every new life.)</span>');
      if (TB.RunCard && TB.RunCard.chips) t.splice(1, 0, TB.RunCard.chips(s)); // the life, in one chip row, under the title
      return t;
    },
    choices: [
